...
bool UdpSocket::start_receiving(Task::BaseConfig &task_config,
  ...
  // DODATO NAKNADNO
  int sndbuf = 128 * 1024;  // 128 KB send buffer
  if (setsockopt(socket_, SOL_SOCKET, SO_SNDBUF, &sndbuf, sizeof(sndbuf)) < 0) {
    logger_.warn("UdpSocket: SO_SNDBUF failed: {}", error_string());
  }
  int rcvbuf = 64 * 1024;   //  64 KB recv buffer
  if (setsockopt(socket_, SOL_SOCKET, SO_RCVBUF, &rcvbuf, sizeof(rcvbuf)) < 0) {
    logger_.warn("UdpSocket: SO_RCVBUF failed: {}", error_string());
  }
